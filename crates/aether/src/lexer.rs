...
pub enum TokenKind {
    ...
    Let, Mut, Fn, Struct, Enum, Match, If, Else, While, For, In, Loop, Break, Continue, Return, True, False,
    Plus, Minus, Star, Slash, Percent,
    Eq, EqEq, Bang, BangEq, Lt, Le, Gt, Ge, AndAnd, OrOr,
    LParen, RParen, LBrace, RBrace, LBracket, RBracket,
    Comma, Semicolon, Colon, ColonColon, Dot, Arrow, FatArrow,
    Eof,
}
...
pub fn lex(source: &str) -> Result<Vec<Token>, String> {
    ...
    while i<chars.len(){let c=chars[i]; if c.is_whitespace(){if c=='\n'{line+=1;col=1}else{col+=1}i+=1;continue} if c=='/'&&chars.get(i+1)==Some(&'/'){while i<chars.len()&&chars[i]!='\n'{i+=1;col+=1}continue}
        let sl=line;let sc=col;let kind=match c{
            ...
            ':'=>{i+=1;col+=1;if chars.get(i)==Some(&':'){i+=1;col+=1;TokenKind::ColonColon}else{TokenKind::Colon}},'.'=>{i+=1;col+=1;TokenKind::Dot},
            ...
            '='=>{i+=1;col+=1;if chars.get(i)==Some(&'='){i+=1;col+=1;TokenKind::EqEq}else if chars.get(i)==Some(&'>'){i+=1;col+=1;TokenKind::FatArrow}else{TokenKind::Eq}},
            '/'=>{i+=1;col+=1;TokenKind::Slash},'!'=>{i+=1;col+=1;if chars.get(i)==Some(&'='){i+=1;col+=1;TokenKind::BangEq}else{TokenKind::Bang}},
            '<'=>{i+=1;col+=1;if chars.get(i)==Some(&'='){i+=1;col+=1;TokenKind::Le}else{TokenKind::Lt}},'>'=>{i+=1;col+=1;if chars.get(i)==Some(&'='){i+=1;col+=1;TokenKind::Ge}else{TokenKind::Gt}},
            ...
            c if c.is_ascii_alphabetic()||c=='_'=>{let b=i;while i<chars.len()&&(chars[i].is_ascii_alphanumeric()||chars[i]=='_'){i+=1;col+=1}match chars[b..i].iter().collect::<String>().as_str(){"let"=>TokenKind::Let,"mut"=>TokenKind::Mut,"fn"=>TokenKind::Fn,"struct"=>TokenKind::Struct,"enum"=>TokenKind::Enum,"match"=>TokenKind::Match,"if"=>TokenKind::If,"else"=>TokenKind::Else,"while"=>TokenKind::While,"for"=>TokenKind::For,"in"=>TokenKind::In,"loop"=>TokenKind::Loop,"break"=>TokenKind::Break,"continue"=>TokenKind::Continue,"return"=>TokenKind::Return,"true"=>TokenKind::True,"false"=>TokenKind::False,x=>TokenKind::Ident(x.into())}},
