...
pub enum TokenKind {
    ...
    Let, Mut, Fn, If, Else, While, For, In, Return, True, False,
...
pub fn lex(source: &str) -> Result<Vec<Token>, String> {
    ...
    while i<chars.len(){let c=chars[i]; if c.is_whitespace(){if c=='\n'{line+=1;col=1}else{col+=1}i+=1;continue} if c=='/'&&chars.get(i+1)==Some(&'/'){while i<chars.len()&&chars[i]!='\n'{i+=1;col+=1}continue}
        let sl=line;let sc=col;let kind=match c{
            ...
            '|'=>{i+=1;col+=1;if chars.get(i+1)==Some(&'|'){i+=2;col+=2;TokenKind::OrOr}else if chars.get(i)==Some(&'|'){i+=1;col+=1;TokenKind::OrOr}else{return Err(format!("unexpected '|' at {sl}:{sc}"))}},
