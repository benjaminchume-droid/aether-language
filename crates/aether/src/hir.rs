...
#[derive(Clone,Debug,PartialEq)] pub enum Expr{Int(i64),Float(f64),Bool(bool),Str(String),Array(Vec<Expr>),Tuple(Vec<Expr>),Range{start:Box<Expr>,end:Box<Expr>,inclusive:bool},Ident(String),StructInit{name:String,fields:Vec<(String,Expr)>},Member{target:Box<Expr>,field:String},EnumValue{enum_name:String,variant:String,payload:Vec<Expr>},Index{target:Box<Expr>,index:Box<Expr>},Unary{op:UnaryOp,expr:Box<Expr>},Binary{left:Box<Expr>,op:BinaryOp,right:Box<Expr>},Call{callee:Box<Expr>,args:Vec<Expr>}}
