...
#[derive(Clone, Debug, PartialEq)] pub enum Item{Let{name:String,mutable:bool,annotation:Option<Type>,value:Expr},Fn{name:String,params:Vec<Param>,return_type:Option<Type>,body:Vec<Stmt>},Struct{name:String,fields:Vec<Field>},Enum{name:String,variants:Vec<String>},Stmt(Stmt)}
#[derive(Clone, Debug, PartialEq)] pub struct MatchArm{pub pattern:Pattern,pub body:Vec<Stmt>}
#[derive(Clone, Debug, PartialEq)] pub enum Pattern{Wildcard,Enum{enum_name:String,variant:String}}
...
#[derive(Clone, Debug, PartialEq)] pub enum Expr{Int(i64),Float(f64),Bool(bool),Str(String),Array(Vec<Expr>),Ident(String),StructInit{name:String,fields:Vec<(String,Expr)>},Member{target:Box<Expr>,field:String},EnumValue{enum_name:String,variant:String},Index{target:Box<Expr>,index:Box<Expr>},Unary{op:UnaryOp,expr:Box<Expr>},Binary{left:Box<Expr>,op:BinaryOp,right:Box<Expr>},Call{callee:Box<Expr>,args:Vec<Expr>}}
