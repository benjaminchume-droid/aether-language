...
#[derive(Debug,Clone,PartialEq)] pub struct Program{pub items:Vec<Item>}
...
#[derive(Debug,Clone,PartialEq)] pub enum Expr{Int(i64),Float(f64),Bool(bool),Str(String),Array(Vec<Expr>),Tuple(Vec<Expr>),Range{start:Box<Expr>,end:Box<Expr>,inclusive:bool},Ident(String),StructInit{name:String,fields:Vec<(String,Expr)>},Member{target:Box<Expr>,field:String},EnumValue{enum_name:String,variant:String,payload:Vec<Expr>},Index{target:Box<Expr>,index:Box<Expr>},Unary{op:UnaryOp,expr:Box<Expr>},Binary{left:Box<Expr>,op:BinaryOp,right:Box<Expr>},Call{callee:Box<Expr>,args:Vec<Expr>}}
#[derive(Debug,Clone,Copy,PartialEq,Eq)] pub enum UnaryOp{Neg,Not}
#[derive(Debug,Clone,Copy,PartialEq,Eq)] pub enum BinaryOp{Add,Sub,Mul,Div,Mod,Eq,Ne,Lt,Le,Gt,Ge,And,Or}
